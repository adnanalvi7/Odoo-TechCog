...
class ZKTecoAttendance(models.Model):
    ...
    def fetch_attendance(self):
        ...
        odoo_api_url = "https://adnanalvi7-odoo-techcog.odoo.com/biometric/attendance"  # Replace with your actual Odoo API URL
        ...
        try:
            ...
            for log in logs:
                _logger.info(f"Fetching attendance: User ID {log.user_id}, Time {log.timestamp}, Status: {log.status}, Punch: {log.punch}")
                ...
                if employee:
                    ...
                    # Send attendance data to Odoo API
                    attendance_data = {
                        'employee_id': employee.id,
                        'attendance_time': str(atten_time),
                        'status': log.status,
                        'device_uid': log.user_id,
                    }

                    try:
                        response = requests.post(odoo_api_url, json=attendance_data)
                        if response.status_code == 200:
                            _logger.info(f"Successfully sent attendance data for Employee {employee.name}")
                        else:
                            _logger.error(f"Failed to send attendance data for Employee {employee.name}. Status Code: {response.status_code}")
                    except Exception as e:
                        _logger.error(f"Error sending attendance data for Employee {employee.name}: {str(e)}")
